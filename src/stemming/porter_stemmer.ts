...
{  
   b: any[];
   i: number;     /* offset into b */
   i_end: number; /* offset to end of stemmed word */
   j: any;
   k: any;
   ...
   {  if (this.i == this.b.length)
      {  
         let new_b: any[] = [];
         for (let c = 0; c < this.i; c++) new_b.push(this.b[c]);
         this.b = new_b;
      }
      this.b.push(ch);
      this.i++;
    ...
    */
    ...
    {  
      ...
      {  let new_b = [];
         for (let c = 0; c < this.i; c++) new_b[c] = this.b[c];
         this.b = new_b;
      }
   ...
   */
   ...
   vowelinstem(): boolean
   {  let i; for (i = 0; i <= this.j; i++) if (! this.cons(i)) return true;
      return false;
   }

   /* doublec(j) is true <=> j,(j-1) contain a double consonant. */
   doublec(j: number): boolean
   {  if (j < 1) return false;
   ...
   */
   ...
   {  let l = s.length;
      let o = this.k-l+1;
      if (o < 0) return false;
      for (let i = 0; i < l; i++) if (this.b[o+i] != s[i]) return false;
      this.j = this.k-l;
      return true;
   }
   ...
   r(s: string) { if (this.m() > 0) this.setto(s); }
   ...
   */
   ...
   step2() { if (this.ends("y") && this.vowelinstem()) this.b[this.k] = 'i'; }

   /* step3() maps double suffices to single ones. so -ization ( = -ize plus
      -ation) maps to -ize etc. note that the string before the suffix must give
      m() > 0. */

   step3() { if (this.k == 0) return; /* For Bug 1 */ switch (this.b[this.k-1])
   {
   ...
   step5()
   {   if (this.k == 0) return; /* for Bug 1 */ switch (this.b[this.k-1])
   ...
   stem()
   {  this.k = this.i - 1;
      if (this.k > 1) { this.step1(); this.step2(); this.step3(); this.step4(); this.step5(); this.step6(); }
